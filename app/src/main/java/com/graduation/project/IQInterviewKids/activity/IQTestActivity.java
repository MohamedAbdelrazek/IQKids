...
public class IQTestActivity extends AppCompatActivity {
    ViewPager pager;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        totalQuestions = MyServerData.getInstance().getTotalNumberIqQuestions();
        ...
        pager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageScrollStateChanged(int state) {
                int totalQuestions = MyServerData.getInstance().getTotalNumberIqQuestions();
    ...
    public void FinishTest(View v) {
        ...
        if (MyServerData.getInstance().getTestState().equals("inProgress")) {
            ...
            Question[] allQuestions = MyServerData.getInstance().getAllIqQuestions();
    ...
    private int getTotalCorrectAnswer() {
        ...
        for (int i = 0; i < MyServerData.getInstance().getAllIqQuestions().length; i++) {
            Boolean isCorrect = Arrays.equals(MyServerData.getInstance().getAllIqQuestions()[i].getAllCorrectAnswers(), MyServerData.getInstance().getAllIqQuestions()[i].getUserAnswers());
            if (isCorrect) {
                total++;
            }
        }
